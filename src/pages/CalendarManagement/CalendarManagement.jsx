...
//
//     // Phân loại lịch: có lớp tín chỉ hoặc không có
//     schedules.forEach((schedule) => {
...
//     const result = [];
//
//     // Xử lý các cụm lịch có lớp tín chỉ
//     Object.values(groupedByCreditClass).forEach((group) => {
//       // Lấy danh sách các ID trong cụm
//       const clusterIds = group.map((s) => s.calendarId).join(",");
//
//       group.forEach((schedule, index) => {
//         result.push({
//           ...schedule,
//           uniqueKey: schedule.calendarId,
//           isFirstInGroup: index === 0,
//           groupSize: group.length,
//           clusterIds: clusterIds, // Thêm thuộc tính chứa chuỗi ID của cụm
//         });
//       });
//     });
//
//     // Thêm các lịch đơn (lịch mượn phòng)
//     singleSchedules.forEach((schedule) => {
//       result.push({
//         ...schedule,
//         uniqueKey: schedule.calendarId,
//         isFirstInGroup: true, // Mỗi lịch đơn là một "cụm" 1 thành viên
//         groupSize: 1,
//         clusterIds: schedule.calendarId, // ID của chính nó
//       });
//     });
//
//     return result;
